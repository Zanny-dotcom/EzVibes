...
- `renderer/app.js` — folder browser UI, context menu, session state, terminal popup lifecycle, narration sidebar logic. One IIFE, ~1100 lines.
...
- `tab-plan.md` — design + step-by-step implementation outline for Chrome-style tabs per session window. **Approved, not yet implemented.**
...
Terminal sessions are stored in `main.js` in a `Map` keyed by renderer-created `sessionId`. Renderer keeps matching state in:

- `sessionsByPath`
- `sessionsById`

(The `tab-plan.md` proposal renames these to `windowsByPath` and `tabsById` once tabs land.)
...
- Right-clicking a folder opens actions (`Launch Claude`, minimize/restore when a session exists, rename, etc.).
- Right-clicking a file has no real actions yet.
- `Launch Claude Here` launches a Claude session for the current directory.
- Narration sidebar ("What Was Made") toggles open via the header button. It shows a per-folder activity log for any Claude session that has run in that folder, plus a session summary.
...
- Multiple Claude instances per folder (tabs) not yet implemented — see `tab-plan.md`.
