...
> Written so the session can be cleared and resumed cleanly. The canonical project doc is **`CLAUDE.md`**; the full improvement roadmap is **`inbox/ezvibes-comprehensive-improvement-plan-2026-05-28.md`**. This file summarizes what changed this session and what's left.
...
2. **`summary_recap.md`** (this file) — the findings ledger and prioritized next steps.
...
- **Markdown routing:** per the global `CLAUDE.md` rule, generated `.md` files go to `inbox/` with kebab-case names. This recap is at the project root by explicit request; an identical copy lives at `inbox/session-recap-p0-safety-fixes-2026-05-29.md`.
